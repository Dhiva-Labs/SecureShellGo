...
// (lib/services/layout_breakpoints.dart, >=600dp) switches on, so the window
// is not allowed to shrink past it.
...
FlutterWindow::MessageHandler(HWND hwnd, UINT const message,
  ...
  switch (message) {
    ...
    case WM_GETMINMAXINFO: {
      ...
      info->ptMinTrackSize.x = static_cast<LONG>(kMinimumWidth * scale_factor);
      info->ptMinTrackSize.y =
          static_cast<LONG>(kMinimumHeight * scale_factor);
      return 0;
    }
  }
